    ...
    public function datatable()
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                $edit   = '<a href="' . url("master/menus/{$row->id}/edit") . '" class="btn btn-sm btn-warning me-1"><i class="ri ri-edit-line"></i> Edit</a>';
                $delete = '<button class="btn btn-sm btn-danger btn-delete" data-id="' . $row->id . '" data-url="' . url("master/menus/{$row->id}") . '"><i class="ri ri-delete-bin-line"></i> Hapus</button>';
                return $edit . $delete;
            })
            ->rawColumns(['action'])
            ->make(true);
